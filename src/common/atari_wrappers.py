...
def wrap_deepmind(env, episode_life=True, clip_rewards=True, frame_stack=True, scale=False):
    ...
    env = TransposeImageObs(env, op=[2, 0, 1]) 
    return env
...
class TransposeImageObs(gym.ObservationWrapper):
    ...
    def observation(self, obs):
        return np.transpose(obs, (self.op[0], self.op[1], self.op[2]))

"""Can be improved with vectorized environment (idk so many implementations)"""
